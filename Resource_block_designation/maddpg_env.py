...
class radioenv(object):
    ...
    def Reset(self, agents, perm, xCords, yCords):
        ...
        #xCords = [28, 0, 450, -600]#[28,   0,450,-600]
        #yCords = [-96, -250, 176, 800]#[-96,-250,176, 800]
        self.rb_perm = perm
        for index, UE in enumerate(agents):
            ...
            ls = np.array(self.rb_perm[index])
            ls[ls==100] = -10
            ls = ls.tolist()
            ...
            self.observation_list[index].extend(ls)
    ...
    def calculate_recived_transmision_power(self, agents):
        ...
        for RB_index in np.arange(self.resource_blocks):
            for index, UE in enumerate(agents): 
                UE.rx[RB_index] = (self._compute_receive(UE,UE.tx_powers[RB_index], index, RB_index))
    ...
    def calculate_reward(self, agents):
        ...
        std_reward = self._watt_to_dbm(np.std(self.sinr_subChannels))
        mean_reward = self._watt_to_dbm(np.mean(self.sinr_subChannels))
        reward = (self.alpha*mean_reward) - (self.beta*std_reward)
        return 1/reward 
    ...
    def _compute_receive(self, ue_obj, tx_power, ue_ind, rb_ind):
        ...
        signal = transmitter_Power + tx_antenna_gain - (self.rb_perm[ue_ind][rb_ind]*free_Space_Path_Loss) + rx_Antenna_Gain
    ...
    def step(self, actions, agents):
        ...
        for index, UE in enumerate(agents):
            ...
            for index_b in np.arange(self.resource_blocks):
                bit = round(actions[index][index_b])
        ...
        for index, UE in enumerate(agents):
            ...
            ls[ls==100] = -10
            ls = ls.tolist()
            self.next_observation_list[index].extend(ls)
